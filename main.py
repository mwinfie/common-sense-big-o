...
def main():
    ...
    fig, axs = plt.subplots(1, 2, figsize=(12, 5 * 1), squeeze=False)
    ...
    axs[0, 0].scatter(n_sizes, y_time, color='blue', label='Measured Time', alpha=0.6)
    axs[0, 0].plot(n_sizes, theory_time_raw * scale_t, color='red', linestyle='--', label=f'{time_label}')
    axs[0, 0].set_title(f"{test.name}: Time Complexity")
    ...
    axs[0, 1].scatter(n_sizes, y_space, color='orange', label='Measured Space', alpha=0.6)
    axs[0, 1].plot(n_sizes, theory_space_raw * scale_s, color='green', linestyle='--', label=f'{space_label}')
    axs[0, 1].set_title(f"{test.name}: Space Complexity")
    axs[0, 1].set_xlabel("N")
    axs[0, 1].set_ylabel("Peak Bytes")
    axs[0, 1].legend()
